...
export default function MsmeDashboard() {
  ...
  const profile = useMemo(
    () => buildDealProfile(selectedDeal || {}),
    [selectedDeal]
  );
  const financialMetrics = useMemo(
    () => computeFinancialMetrics(selectedDeal || {}),
    [selectedDeal]
  );
  const latestInvestments = useMemo(
    () => investments.slice(0, 2),
    [investments]
  );
  ...
  const msmeKpis = [
    ...
    },
    {
      ...
      type: "text",
      helper:
        financialMetrics.runway_months != null
          ? "Based on cash / burn rate"
          : "Not enough data",
    },
    {
      label: "LTV/CAC ratio",
    ...
    },
    {
      ...
      helper:
        financialMetrics.customer_growth_rate != null
          ? `Growth rate ${formatPercent(
              financialMetrics.customer_growth_rate,
              1
            )}`
          : null,
    },
  ...
  const handleContactSave = async (e) => {
    ...
    try {
      ...
      setSelectedDeal((prev) =>
        prev ? { ...prev, ...(updatedDeal || payload) } : prev
      );
      setContactStatus({
        saving: false,
        error: "",
        success: "Business contact information updated.",
      });
    } catch (err) {
      const message =
            ...
            <div className="rounded-2xl border border-[#E5E7EB] bg-white p-5 shadow-sm shadow-[#E0E7FF] flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-lg font-semibold text-[#0F172A]">
                  Add your startup via the MSME wizard
                </p>
                <p className="text-sm text-[#4B5563]">
                  Share your business profile to unlock tailored deals and
                  financing.
                </p>
              </div>
              <Link
        ...
        {(hasStartup || selectedDeal || loadingDeal) && (
          <>
            ...
            {!loadingDeal && !selectedDeal && !dealError && (
              <div className="rounded-2xl border border-dashed border-[#CBD5E1] bg-white p-6 text-sm text-[#4B5563] shadow-sm shadow-[#E0E7FF]">
                No MSME deal found. Launch the wizard to list your startup and
                view live metrics.
              </div>
            )}
                  ...
                  <div className="lg:col-span-2 rounded-2xl border border-[#E5E7EB] bg-white p-6 shadow-md shadow-[#E0E7FF]">
                    ...
                    <div className="mt-6 grid gap-4 md:grid-cols-2">
                      <div className="rounded-xl border border-[#E5E7EB] bg-[#F8FAFC] p-4">
                        ...
                        <p className="text-sm text-[#4B5563] mt-1">
                          Utilized:{" "}
                          {profile.utilizedLabel || "Pending drawdown"}
                        </p>
                      </div>
                      <div className="rounded-xl border border-[#E5E7EB] bg-[#F8FAFC] p-4">
                        <p className="text-sm font-semibold text-[#0F172A]">
                ...
                <section className="rounded-2xl border border-[#E5E7EB] bg-white p-6 shadow-md shadow-[#E0E7FF] space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-[#4B5563]">
                        Calculated per deal from your latest financial inputs.
                      </p>
                    </div>
                ...
                <section className="rounded-2xl border border-[#E5E7EB] bg-white p-6 shadow-sm shadow-[#E0E7FF] space-y-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-sm text-[#4B5563]">
                        Update the contact details investors will see.
                        Registered address is read-only.
                      </p>
                    </div>
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="rounded-xl border border-[#E5E7EB] bg-[#F8FAFC] p-4">
                      ...
                      <div className="mt-2 flex items-start gap-2 text-sm text-[#0F172A]">
                        ...
                        <span>
                          {selectedDeal?.registeredAddress || "Not provided"}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <section className="grid gap-4 lg:grid-cols-3">
                  <div className="lg:col-span-2 rounded-2xl border border-[#E5E7EB] bg-white p-6 shadow-md shadow-[#E0E7FF]">
                    ...
                    <Link
                      to={
                        selectedDeal?._id
                          ? `/deals/${selectedDeal._id}/cashflows`
                          : "/deals"
                      }
                      ...
                      latestInvestments.map((inv) => (
                        ...
                        >
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-xs text-[#1F6FEB]">
