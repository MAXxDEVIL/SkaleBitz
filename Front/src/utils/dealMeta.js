...
export const normalizeStatus = (status) => (status ? String(status).trim().toLowerCase() : '');
...
export const getStatusMeta = (status) => {
  ...
  const label = normalized ? `${normalized.charAt(0).toUpperCase()}${normalized.slice(1)}` : 'Offline';
...
export const resolveRiskLabel = (deal = {}, fallback = 'On track') => deal.risk || fallback;
...
export const buildDealProfile = (deal = {}) => {
  ...
  const deriveUtilizationPct = () => {
    if (deal.performance?.utilizationPct != null) return deal.performance.utilizationPct;
    if (!facilitySizeValue) return null;
    return Math.min(100, Math.round(((utilizedValue || 0) / facilitySizeValue) * 100));
  };
    ...
    ? deal.cashflows
        ...
        .map((entry, idx) => {
          return {
            ...
            date: formatDateLabel(entry.dueDate || entry.date || entry.createdAt),
            amount:
  ...
  const delinquencyValue = deal.performance?.delinquencyRate ?? deal.delinquency;
  ...
  return {
    ...
    performance: {
      ...
      delinquency:
        typeof delinquencyValue === 'number' ? formatPercent(delinquencyValue, 1) : delinquencyValue || '—',
