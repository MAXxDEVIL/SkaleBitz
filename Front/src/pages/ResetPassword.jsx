...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    setStatus("");
    if (!email.trim()) {
      setError("Enter the email you use for SkaleBitz.");
      return;
    }
    try {
      setLoading(true);
      ...
      setStatus(
        data?.message || "If that account exists, we sent a reset link."
      );
    } catch (err) {
      const apiError = err.response?.data?.error || err.response?.data?.message;
      setError(apiError || "Unable to send reset link right now.");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="relative rounded-3xl border border-[#E5E7EB] bg-white p-10 shadow-2xl shadow-[#E0E7FF] space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[#4B5563]">
                Enter the email you use for SkaleBitz. We’ll email a secure
                reset link.
              </p>
            </div>
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              {status && (
                <p className="text-sm font-semibold text-[#16A34A]">{status}</p>
              )}
