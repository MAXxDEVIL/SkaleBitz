...
const sanitizeNumberInput = (value) => {
  ...
  const decimal = sanitized.slice(firstDot + 1).replace(/\./g, "");
  return decimal ? `${integer}.${decimal}` : integer;
};
const getFocusableElements = (root) =>
  root ? root.querySelectorAll(FOCUSABLE_SELECTOR) : [];
import {
  ArrowLeft,
  BarChart3,
  Calendar,
...
export default function InvestorDealDetail() {
  ...
  const financialMetrics = useMemo(
    () => computeFinancialMetrics(deal || {}),
    [deal]
  );
  const nextCashflow = useMemo(
    () =>
      profile.cashflows.find((c) => c.status?.toLowerCase() !== "settled") ||
      profile.cashflows[0],
    [profile.cashflows]
  );
  ...
  const handleInvestConfirm = async () => {
    ...
    const decimals = investAmount.includes(".")
      ? investAmount.split(".")[1]?.length ?? 0
      : 0;
    if (decimals > 2) {
    ...
    if (remainingCapacity != null && numericAmount > remainingCapacity) {
      setInvestError(
        `Amount exceeds remaining capacity of ${formatInvestAmount(
          remainingCapacity
        )}.`
      );
      return;
    ...
    if (numericAmount > MAX_INVESTMENT_AMOUNT) {
      setInvestError(
        `Please enter an amount under ${formatInvestAmount(
    ...
    try {
      const { investment, user: updatedUser } = await allocateInvestment(
        dealId,
        numericAmount
      );
      if (updatedUser) {
      ...
      setDeal((prev) => {
        ...
        return {
          ...prev,
          utilizedAmount: nextUtilized,
          remainingCapacity: nextRemaining,
        };
      });
      setInvestFeedback(
        `Funding confirmed for ${formatInvestAmount(numericAmount)}${
          investment?._id ? ` (Ref: ${investment._id})` : ""
        }.`
      );
      closeInvestModal();
    } catch (err) {
      const message =
        err?.response?.data?.message || "Unable to allocate funds right now.";
  ...
  const canAllocate =
    user?.accountType === "investor" && (remainingCapacity ?? 1) > 0;

  const profitMetrics = [
    {
      label: "Total revenue",
      value: financialMetrics.total_revenue,
      type: "currency",
    },
    {
      label: "Total expenses",
      value: financialMetrics.total_expenses,
      type: "currency",
    },
    {
      label: "Net profit / loss",
      value: financialMetrics.net_profit_loss,
      type: "currency",
    },
    {
      label: "Profit margin",
    ...
    {
      label: "Expense ratio",
      value: financialMetrics.expense_ratio,
      type: "percent",
    },
    {
      label: "Avg monthly revenue",
      value: financialMetrics.avg_monthly_revenue,
      type: "currency",
    },
    {
      label: "Avg monthly expenses",
      value: financialMetrics.avg_monthly_expenses,
      type: "currency",
    },
    {
      label: "Profitability health",
      value: financialMetrics.profitability_health,
      type: "text",
    },
  ];
  ...
  const survivalMetrics = [
    ...
    },
    {
      ...
      value: financialMetrics.survival_probability,
      type: "percent",
    },
  ];
  ...
  const growthMetrics = [
    ...
    },
    {
      ...
      value: financialMetrics.marketing_efficiency,
      type: "text",
    },
    {
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="mt-3 flex flex-wrap gap-3 text-xs">
                  <span
                    className={`rounded-full px-3 py-1 font-semibold ${profile.statusMeta.className}`}
                  >
              ...
              <div className="rounded-xl border border-[#E5E7EB] bg-[#F8FAFC] p-4">
                ...
                <p className="text-sm text-[#4B5563] mt-1">
                  ...
                  {remainingCapacity != null
                    ? formatInvestAmount(remainingCapacity)
                    : "Pending"}
                </p>
              </div>
              <div className="rounded-xl border border-[#E5E7EB] bg-[#F8FAFC] p-4">
              ...
              <div className="mt-4 space-y-3 text-sm text-[#4B5563]">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-[#0F172A]">
                    {profile.performance.tenor}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>DSO (days)</span>
                  <span className="font-semibold text-[#0F172A]">
                    {profile.performance.dso}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Delinquency</span>
                  <span className="font-semibold text-[#10B981]">
                    {profile.performance.delinquency}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Realized MOIC</span>
                  <span className="font-semibold text-[#0F172A]">
                    {profile.performance.realizedMoic}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Utilization</span>
                  <span className="font-semibold text-[#1F6FEB]">
                    {profile.performance.utilization}
                  </span>
                </div>
              </div>
            </div>
              ...
              <ul className="mt-4 space-y-2 text-sm text-[#4B5563]">
                ...
                <li className="flex items-center gap-2">
                  <CheckCircle2
                    size={16}
                    className={
                      profile.riskControls.payoutMonitoring
                        ? "text-[#10B981]"
                        : "text-[#B91C1C]"
                    }
                  />
                ...
                <li className="flex items-center gap-2">
                  <CheckCircle2
                    size={16}
                    className={
                      profile.riskControls.diversification
                        ? "text-[#10B981]"
                        : "text-[#B91C1C]"
                    }
                  />
        ...
        <section className="rounded-2xl border border-[#E5E7EB] bg-white p-6 shadow-md shadow-[#E0E7FF]">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-[#4B5563]">
                Calculated in real time from the MSME&apos;s submitted
                financials.
              </p>
            </div>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {financialMetricGroups.map((group) => (
              <div
                key={group.title}
                className="rounded-xl border border-[#E5E7EB] bg-[#F8FAFC] p-4 shadow-sm shadow-[#E0E7FF]"
              >
            ...
            <div className="mt-4 space-y-3">
              {documents.map((doc) => (
                ...
                >
                  ...
                  <button className="text-sm font-semibold text-[#1F6FEB] hover:underline">
                    View
                  </button>
                </div>
              ))}
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-[#4B5563] mt-1">
                  Enter the amount you want to commit. This flow simulates
                  funding for review and does not move funds.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-4 space-y-2">
              ...
              <p className="text-xs text-[#4B5563]">
                Available balance:{" "}
                {formatInvestAmount(Number(user?.balance ?? 0))}
              </p>
              {remainingCapacity != null && (
                <p className="text-xs text-[#4B5563]">
              ...
              {investError && (
                <p className="text-sm text-[#B91C1C]">{investError}</p>
              )}
            </div>
